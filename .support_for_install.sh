...
#   read answer
   ...
   if [ $dyninstver == "20171128" -o $dyninstver == "20180414" ]; then
   ...
   if [ "$use_rpm" = 1 ] ; then
     ...
     if [ "$KRELL_ROOT_TARGET_ARCH" == "bgp" ]; then
       ...
       echo "cmake .. -DCMAKE_INSTALL_PREFIX=$RPM_BUILD_ROOT/$KRELL_ROOT_PREFIX -Dcap_dwarf=1  -DINSTALL_LIB_DIR=$RPM_BUILD_ROOT/$KRELL_ROOT_PREFIX/$LIBDIR -DINSTALL_INCLUDE_DIR=$RPM_BUILD_ROOT/$KRELL_ROOT_PREFIX/include/dyninst -DCMAKE_PREFIX_PATH=$RPM_BUILD_ROOT/$KRELL_ROOT_PREFIX -DCMAKE_BUILD_TYPE=RelWithDebInfo -DLIBDWARF_LIBRARIES=$LIBDWARF_LIBNAME -DLIBDWARF_INCLUDE_DIR=$LIBDWARFINC -DLIBELF_LIBRARIES=$LIBELF_LIBNAME -DLIBELF_INCLUDE_DIR=$LIBELFINC -DPATH_BOOST=$DYNINST_BOOST_ROOT -DIBERTY_LIBRARIES=$LIBIBERTY_LIBNAME -DIBERTY_LIBRARY=$LIBIBERTY_LIBNAME -DBUILD_RTLIB_32=OFF -DCHECK_RTLIB_32=OFF" > ./build_dyninst_cmake.txt
	  ...
	  if [ "$KRELL_ROOT_TARGET_ARCH" == "mic" ]; then
              ...
              if [ "$answer" = Y -o "$answer" = y ]; then
                ...
                if [ "$use_rpm" = 1 ] ; then
                  ...
                  if [ $dyninstver == "9.3.2" -o $dyninstver == "20171128" -o $dyninstver == "20180414" ]; then
